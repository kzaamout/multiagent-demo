...
**1.4 Tool descriptions say when to use a tool and when not to.** Not built. Ours say what a tool does and stop there: "Total counts and lengths, apply the waste factors, and roll up labour hours." None says that a reply is refused if the tool was not called, or that the seat must not do the arithmetic itself. The research's example is the pattern: a bare "Calculate margin" against a description naming the preconditions and the cases it is wrong for. Rewrite the descriptions for `quantity_calculate`, `price_list_lookup` and `vision_read_drawing`, each saying when to call it, what must be true first, and what not to use it for. Targets 23 tool-skipped refusals across the Estimator and Pricing.

**1.5 A graded checklist item carries its clarification.** Not built. Intake's largest failure by far, 83 refusals, is "items are not pass but there are 0 clarifications". The checklist already states a default for every item that has one. When Intake grades an item not-pass and raises no clarification, the engine can build the clarification stub from the checklist entry and hand it back, or accept the grade and attach the default itself. The judgment that stays with the model is the grade; the paperwork that follows from the grade does not need a model.
