...
def brief(
    project: str, deadline: str | None = "3 Nov", spec: str | None = "Division 26, 36 pages"
) -> dict[str, Any]:
    data: dict[str, Any] = {
        "project": project,
...
def estimator_steps(b: PromptBundle, start: int, lines: int = 38) -> list[Emit]:
    return [
        progress(
            ...
            "t1",
            start,
            "Reading single-line E-001 and panel schedules E-101 to E-102.",
            b,
...
def pricing_steps(
    ...
    summary = f"{priced} of {lines} lines priced" + (
        f" · {len(exc)} exception: {exc[0]}" if exc else " · no exceptions"
    )
    return [
        progress(
            "pricing",
            "t2",
            start,
            f"Pricing {lines} lines from the supplier fixture.",
            b,
            meter(3500, 0.0, 5000),
            headline=f"Pricing the BOM, {lines} lines",
        ),
        tool(
            "pricing",
            "t2",
            ...
            "price_list.lookup",
            f"{lines} item codes",
            f"{priced} found" + (f", {len(exc)} missing" if exc else ""),
            110,
            b,
        ),
        completed(
...
def reviewer_steps(b: PromptBundle, offset: int, findings: list[dict[str, Any]], summary: str) -> list[Emit]:
