...
Two workflows run on the same engine: responding to an electrical RFP, and producing an appraisal report from an intake order. Same agents in the same seats, different trade knowledge. That is the point: the pattern is reusable, the domain is a configuration.
