...
HEADER_NAV = Rect(1340, 8, 1910, 48, "build stamp added to every header and pre-flight dot pending (decision)", "S7")
DRY_INTAKE = Rect(536, 208, 760, 256, "Dry intake toggle added to the composer (pre-S1 decision 2)", "S3")
ARTIFACT_BODY = Rect(1280, 316, 1896, 940, "artifact panel placeholder; compiled pages and Edit and Reject arrive in S4", "S4")
DRY_INTAKE_UNDER_BANNER = Rect(536, 440, 760, 488, "Dry intake toggle added to the composer (pre-S1 decision 2)", "S3")
ARTIFACT_BODY_UNDER_BANNER = Rect(1280, 548, 1896, 940, "artifact panel placeholder; compiled pages arrive in S4", "S4")
COMPARE_STRIP = Rect(1280, 264, 1896, 304, "Compare strip content arrives with Single-model mode", "S5")
COMPARISON_LINE = Rect(1586, 960, 1910, 1040, "Team vs Single comparison line arrives with Single-model mode", "S5")
...
MASKS: dict[str, list[Rect]] = {
    ...
    "preflight-pending": [Rect(1340, 8, 1910, 48, "build stamp and pre-flight dot added to the header (decision)", "S7")],
}
