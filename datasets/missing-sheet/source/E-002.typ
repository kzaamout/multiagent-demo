...
#let circuits = (
  ...
  "8": ("Receptacles, lobby (2)", 360, "20A/1P"),
  ...
  "13": ("Spare", 0, "20A/1P"),
  "14": ("Feeder F2 to panel LP-2, see E-003", 0, "100A/3P"),
  "18": ("Feeder F2 to panel LP-2", 0, "3P"),
  "16": ("Feeder F2 to panel LP-2", 0, "3P"),
)
#let row(n) = {
  let key = str(n)
  if key in circuits {
...
#table(
  ...
  h[Panel: LP-1, new], h[Voltage: 120/208 V, 3 phase, 4 wire], h[Bus: 225 A], h[Main: 225 A main breaker],
  c[Type: 42-circuit panelboard], c[Mounting: surface], c[Location: electrical room], c[Fed from: transformer T1, sheet E-001],
  c[Interrupting rating: 10 kA], c[Breakers: bolt-on], c[Circuits in use: 11 and feeder F2], c[Spare breakers: 2, not wired],
)
#v(6pt)
#grid(
...
#grid(
  ...
  table(
    ...
    c[Receptacles, 26 at 180 VA], c[4680],
    c[Feeder F2 to LP-2, load on schedule E-003], c[],
    h[Total connected load on LP-1 circuits], h[6530],
  ),
  [
    ...
    + Ratings agree with the single-line diagram, sheet E-001.
  ],
)
