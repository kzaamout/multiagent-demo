...
13. **Every seat on a local model** (2026-09-15). Before the first live run, the owner moved every seat to Ollama so a run cannot produce a cloud bill: Qwen 3.5 9B for the Orchestrator, Intake, Estimator, and Writer, Llama 3.1 8B for Pricing, and Gemma 4 12B for the Reviewer, a different family from the Writer. Thinking mode is off for speed. The cloud defaults of decision 8 stay defined in `config/models.yaml` for switching back. The Settings page cannot change models until S5, so the choice lives in that file. S2's evidence is recorded on local models; the roadmap outcome's cloud seats need a later run, ideally after S3 adds the cost ceiling.
